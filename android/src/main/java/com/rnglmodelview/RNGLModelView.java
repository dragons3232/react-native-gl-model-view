...
public class RNGLModelView extends GLTextureView implements RendererDelegate {
    ...
    private final String contentOfFile(String uri) {
        InputStream textureStream = null;
        try {
            if (uri.contains("file:/")) {
                textureStream = new FileInputStream(new File(uri.replace("file:/", "")));
            } else {
                String textureFileName = uri.replace(".mtl", "");
                textureStream = getContext().getAssets().open(textureFileName);
            }

            BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(textureStream));
    ...
    public void setModelTextureUri(@Nullable String pUri) {
        ...
        if (pUri != null) {
            ...
            } else if (pUri.endsWith(".mtl")) {
                String content = contentOfFile(pUri);
                if (false && content.contains("map_Kd")) {
                    String line = content.substring(content.indexOf("map_Kd"));
                    line = line.substring(0, line.indexOf("\n")).trim();
                    texture = loadTexture(line.substring(HEADER_URI_ASSETS.length()));
                } else if (content.contains("Kd")) {
                    String line = content.substring(content.indexOf("Kd") + 3);
                    line = line.substring(0, line.indexOf("\n")).trim();
                    String[] parts = line.split(" ");
                    int red = (int) Float.parseFloat(parts[0]) * 255;
                    int green = (int) Float.parseFloat(parts[1]) * 255;
                    int blue = (int) Float.parseFloat(parts[2]) * 255;
                    texture = new Texture(10, 10, new RGBColor(red, green, blue));
                }
            } else {
